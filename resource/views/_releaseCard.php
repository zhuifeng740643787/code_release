<Card v-show="menu.active=='release'">
    ...
    <div class="form-item">
      ...
      <div class="item-content">
        ...
        <div style="margin: 10px 0;">
          <Checkbox v-for="(item, key) in servers" :true-value="item.id" :label="item.id"
                    v-model="formItem.server_ids[key]">{{ item.name }}({{ item.host }})
          </Checkbox>
        </div>
      </div>
    </div>
                ...
                <div class="right-box">
                  <i-select v-model="project.branch_tag" filterable style="width:300px;text-align: left;">
                    <option-group label="分支">
                      <i-option v-for="(branch, branch_key) in project.branches" :value="'branch-' + branch"
                                :key="branch_key">{{ branch }}
                      </i-option>
                    </option-group>
                    <option-group label="标签">
                      <i-option v-for="(tag, tag_key) in project.tags" :value="'tag-' + tag" :key="tag_key">{{ tag }}
                      </i-option>
                    </option-group>
                  </i-select>
                </div>
